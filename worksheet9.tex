...
\question Prove the second De Morgan law in Table 1 by showing that if $A$ and $B$ are sets, then $\overline{A \cup B} = \overline A \cap \overline B$.
\begin{parts}
\part
...
\question Show that if A, B, and C are sets, then $\overline{A \cap B \cap C }= \overline A \cup \overline B \cup \overline C$.
\begin{parts}
\part
by showing each side is a subset of the other side.
\vspace{9in}
